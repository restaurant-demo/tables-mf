  ...
  return (
    <>
      <div className="flex items-center justify-between border-b p-2 bg-vanilla-white rounded-lg shadow-md overflow-hidden relative">
        ...
        <span className="flex-1 px-2">{capacity}</span>
        <span className="flex-1 px-2">{pricePerHour}</span>
        <span
          className="flex-1 px-2 text-sm font-medium 
        {type === 'indoor' ? 'text-blue-600' : type === 'outdoor' ? 'text-yellow-600' : 'text-gray-600'}"
        >
          {type || 'none'}
        </span>
        <span
